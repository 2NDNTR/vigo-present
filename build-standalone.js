...
// Keep the asset manifest in step with what is actually on disk.
try {
...
// Logo artwork is referenced by path in the theme; the single-file build has
// no server, so every /logos/... reference becomes an inline data URI.
for (let i = 0; i < parts.length; i++) {
  parts[i] = parts[i].replace(/(["'])\/logos\/([^"']+)\1/g, (m, q, file) => {
    const abs = path.join(ROOT, 'public/logos', file);
    if (!fs.existsSync(abs)) return m;
    const mime = file.endsWith('.svg') ? 'image/svg+xml' : file.endsWith('.jpg') ? 'image/jpeg' : 'image/png';
    return q + 'data:' + mime + ';base64,' + fs.readFileSync(abs).toString('base64') + q;
  });
}
...
// Brand fonts: the single-file build has no server, so each licensed face is
// embedded directly. Same declarations, same source of truth.
const fontCss = fs.readFileSync(path.join(ROOT, 'app/fonts.css'), 'utf8').replace(
  /url\('\/fonts\/([^']+)'\)/g,
  (m, file) => {
    const abs = path.join(ROOT, 'public/fonts', file);
    if (!fs.existsSync(abs)) return m;
    const mime = file.endsWith('.otf') ? 'font/otf' : file.endsWith('.ttf') ? 'font/ttf' : 'font/woff2';
    return `url(data:${mime};base64,${fs.readFileSync(abs).toString('base64')})`;
  }
);
css = css.replace("@import './fonts.css';", '');
css = fontCss + '\n' + css;
