...
import { authRequired, fetchUser, logout } from '@/lib/auth';
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (e?.message === 'SIGNED_OUT') return router.replace('/login');
        setItems([]);
      }
    })();
  }, [router]);
    ...
    <div onMouseDown={() => setMenu(null)}>
      <header className="topbar">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          {needsAuth && (
            <>
              <button className="btn ghost sm" onClick={() => router.push('/team')}>
                Team
              </button>
              <button
